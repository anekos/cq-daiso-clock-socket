...
```bash
uv run app build              # dist/ に STL を出力
uv run app build --help       # Param の各フィールドがそのままオプション
uv run app build out.stl      # 出力先は位置引数
uv run app interactive        # 各フィールドを対話的に入力してビルド
make build                             # ソース変更を監視して再ビルド (axe)
make watch                             # 同上、ビルドのたびに --show でビューアを開く
```

- `dist/` は CWD 相対に作られるので、必ずプロジェクトルートで実行する。
- `--show` は `cadquery.vis.show` で GUI を開く。**非対話の作業では使わないこと。**
- `--screenshot` は出力ファイルの隣に PNG を保存する。GUI を開かないので非対話の作業でも使える。
- `dist/` の中身を消さない (`rm` 等)。ビルドは同名ファイルを上書きするので、事前に掃除する必要はない。
...
- `src/daiso_clock_socket/__init__.py`
  - `Param`（`click_cadquery.BuildParam` のサブクラス）と `build(param) -> cq.Workplane`。
    `build()` は純粋関数にする (ファイル I/O や表示を持ち込まない)。
  - `click_cadquery.define_app(Param, build)` が `build` / `interactive` を持つ CLI (`main`)
    を組み立てる。`main` は `pyproject.toml` の `project.scripts` から参照されるので、
    変数名を変えない。
- 変わりうる寸法はすべて `Param` のフィールドにする (`build()` の中に数値を直書きしない)。
  `Field(..., description=...)` を書くと `--help` に出る。
- 単位は mm と度。

### フィールドの型の扱い (`define_app` 内部の `define_options`)

- `Literal["a", "b"]` → `click.Choice`。
- `X | None` → `X` を取るオプション (未指定なら `None`)。それ以外の Union は `TypeError`。
- `bool` はフラグではなく値を取る (`--tab true`)。
- `interactive` はフィールドを一つずつ対話的に埋める。Pydantic のバリデーションに落ちたら、
  原因になったフィールドだけ再入力させる (モデルレベルのエラーなら全フィールドを再入力)。

## モデルを変えたら

1. `uv run app build` を通す。
